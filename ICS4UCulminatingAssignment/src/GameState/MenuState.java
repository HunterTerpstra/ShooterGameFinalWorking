...
public class MenuState extends GameState{
	...
	public MenuState(GameStateManager gsm) {//constructor for menu takes in the current game state
		...
		try {
			...
			bg = new Background("/ArenaFloor.jpg"); //loads background
			
			titleColor = new Color(255, 0, 0); //color
			titleFont = new Font ("Century Gothic", Font.PLAIN, 56);
			
			font = new Font ("Arial", Font.PLAIN, 24); //font
	...
	private void select() {
		if (currentChoice == 0) {
			...
			gsm.setState(GameStateManager.LEVEL1STATE); //goes to the level
		}
		if (currentChoice == 1) {
			gsm.setState(GameStateManager.SHOPSTATE); //goes to the shop
		}
		if (currentChoice == 2) {
	...
	//input options for switching selection
